package repository // Repository represents generic interface for interacting with DB
import (
...
type Repository interface {
	...
	GetAllForUsersID(uow *UnitOfWork, out interface{}, tenantID uuid.UUID, preloadAssociations []string) error
...
func (repository *GormRepository) GetAll(uow *UnitOfWork, bank interface{}, preloadAssociations []string) error {
	db := uow.DB
	for _, association := range preloadAssociations {
		db = db.Preload(association)
	}
